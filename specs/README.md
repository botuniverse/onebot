...
CQHTTP 协议是从原 CKYU 平台的 CQHTTP 插件接口修改而来的通用聊天机器人应用接口协议。
...
开发者可以在各类机器人平台实现 CQHTTP 协议，从而使用户可以将基于 CQHTTP 的机器人项目无缝迁移到这些平台。

当需要添加新的功能（例如新的事件、API、消息段类型等）时，可以通过 pull request 扩充本协议，此后其它 CQHTTP 协议的实现在添加同一功能时应参考扩充后的协议。
